...
#define FWNewFeatureImageCount 4
@interface FWFeatureViewController ()
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i<FWNewFeatureImageCount; i++) {
        // 创建UIImageView
        ...
        // 给最后一个imageView添加按钮
        if (i == FWNewFeatureImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置其他属性
    scrollView.contentSize = CGSizeMake(FWNewFeatureImageCount * imageW, 0);
    scrollView.pagingEnabled = YES;
    scrollView.showsHorizontalScrollIndicator = NO;
    scrollView.backgroundColor = FWColor(246, 246, 246);
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = FWNewFeatureImageCount;
    pageControl.centerX = self.view.width * 0.5;
    pageControl.centerY = self.view.height - 30;
    [self.view addSubview:pageControl];
    
    // 2.设置圆点的颜色
    pageControl.currentPageIndicatorTintColor = FWColor(253, 98, 42); // 当前页的小圆点颜色
    pageControl.pageIndicatorTintColor = FWColor(189, 189, 189); // 非当前页的小圆点颜色
...
- (void)start
{
    ...
    /*
      利用push或者modal切换控制器，NewFeatureController都会存在栈里面，浪费内存，由于栈是先进后出想要销毁NewFeatureController必须先要销毁TabBarController不可行，不会销毁因为NewFeatureController在切换控制器是会有一个presentedViewController属性指向TabBarController
     重新设置根控制器，那么NewFeatureController就会自动销毁，没有被使用
     */
    // 切换控制器
